...
CERTS_DIR="./certs"
POSTGRES_CERTS_DIR="./postgres/certs"

# ✅ Make sure directories exist before cd
mkdir -p "$CERTS_DIR"
...
# ✅ Switch back to project root to generate Postgres cert
cd ..

echo "🔹 Generating Postgres TLS certificate..."
openssl genrsa -out "$POSTGRES_CERTS_DIR/server.key" 2048
openssl req -new -key "$POSTGRES_CERTS_DIR/server.key" -out "$POSTGRES_CERTS_DIR/server.csr" \
  -subj "/CN=postgres"
openssl x509 -req -in "$POSTGRES_CERTS_DIR/server.csr" -CA "$CERTS_DIR/ca.crt" -CAkey "$CERTS_DIR/ca.key" -CAcreateserial \
  -out "$POSTGRES_CERTS_DIR/server.crt" -days 365 -sha256

chmod 600 "$POSTGRES_CERTS_DIR/server.key"

echo "✅ All certs generated in $CERTS_DIR and Postgres certs in $POSTGRES_CERTS_DIR"